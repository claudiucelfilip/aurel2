...
# Also check project data dir (for local dev where DATA_DIR is ~/.aurel2)
...
async def api_backtest_comparison():
    """Return 5y and 10y backtest equity curves for production strategy vs SPY.

    Reads pre-computed results from data/backtest_comparison.json
    (generated by scripts/backtest_enhanced.py).
    """
    for path in [BACKTEST_COMPARISON_FILE, BACKTEST_COMPARISON_FILE_ALT]:
        if path.exists():
            try:
                ...
                continue
    return {"error": "No backtest data. Run: python3 scripts/backtest_enhanced.py"}
