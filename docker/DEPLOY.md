...
- `CLAUDE_CONFIG_PATH` (optional): absolute path to a Claude Code config directory outside this repository
...
# Or update the external directory referenced by CLAUDE_CONFIG_PATH,
...
- Confirm `CLAUDE_CONFIG_PATH` points to an external config directory
- Run `docker compose exec aurel2 claude login` if interactive auth is needed
- Restart the aurel2 container
